...
#STEP 1:
./run_slurm_step_local.sh 'Rscript 1_gwas_subset_list.R' &> $LOGS/1_gwas_subset_list.log || exit 1
./run_slurm_step_local.sh './2_extract_regions_from_vcf.sh' &> $LOGS/2_extract_regions_from_vcf.log || exit 1

for chr in $(seq 1 22); do
  ./run_slurm_step_local.sh "Rscirpt 4_colocalize_per_ld_block.R --chr $chr"
done
