...
parser <- argparser::arg_parser("Cluster pairwise coloc results for an LD block")
parser <- argparser::add_argument(
  parser,
  "--ld_block",
  help = "LD block to cluster",
  type = "character"
)
parser <- argparser::add_argument(
  parser,
  "--completed_output_file",
  help = "Sentinel file to write once clustering is complete",
  type = "character"
)
parser <- argparser::add_argument(
  parser,
...
run_post_coloc_clustering <- function(
  coloc_results,
  finemapped_studies,
  ...
  if (can_cluster) {
    ...
    results <- prune_poor_finemapping_results(
      finemapped_studies,
      coloc_results,
      ld_block = ld_block,
      h4_threshold = h4_threshold
    ...
    clustered_results <- cluster_coloc_results(
      filtered_coloc_results,
      finemapped_studies,
      ld_block = ld_block,
      start_time = start_time,
      h4_threshold = h4_threshold
    )
    coloc_results <- mark_false_positives_and_negatives(
      coloc_results,
      clustered_results,
      h4_threshold = h4_threshold
    )
    ...
    message(
      glue::glue(
        "{ld_block}: Marked {sum(coloc_results$false_positive, na.rm = TRUE)} false positives and ",
        "{sum(coloc_results$false_negative, na.rm = TRUE)} false negatives"
      )
    )

    additional_data_per_cluster <- find_snp_and_connectedness_per_cluster(
      clustered_results$groups,
      studies_to_colocalise,
      filtered_coloc_results,
      h4_threshold = h4_threshold
    )
...
prune_poor_finemapping_results <- function(
  finemapped_studies,
  coloc_results,
  ld_block,
  h4_threshold = posterior_prob_h4_threshold
  ...
  coloc_results <- coloc_results |>
    dplyr::mutate(ignore = ignore | study_a == study_b & h4 >= h4_threshold)
  ...
  message(glue::glue("{ld_block}: New number of studies to ignore: {length(bad_unique_studies)}"))
  return(list(finemapped_studies = finemapped_studies, coloc_results = coloc_results))
}

cluster_coloc_results <- function(
  coloc_results,
  finemapped_studies,
  ld_block,
  start_time,
  ...
  h4_adj_mx <- make_adjacency_matrix(coloc_results, h4_threshold = h4_threshold)
  ...
  message(glue::glue("{ld_block}: Pruning graph starting {diff_time_taken(start_time)}"))
  ...
  if (h4_graph_components$no == 1) {
    ...
    if (igraph::vcount(modified_pruned_graph) > 0) {
      ...
      if (nrow(edge_list_names) > 0) {
        for (i in seq_len(nrow(edge_list_names))) {
  ...
  coloc_groups <- data.frame(
    unique_study_id = igraph::vertex_attr(modified_pruned_graph)$name,
    component = igraph::V(modified_pruned_graph)$component
  )
...
mark_false_positives_and_negatives <- function(
  ...
  if (nrow(pruned_edges) > 0) {
    ...
    if (nrow(matches) > 0) {
      ...
      coloc_results$false_positive[
        coloc_results$pair_id %in% matches$pair_id &
          coloc_results$h4 > h4_threshold
      ] <- TRUE
...
make_adjacency_matrix <- function(coloc_results, h4_threshold = posterior_prob_h4_threshold) {
  pvals <- pmax(coloc_results$min_p_study_a, coloc_results$min_p_study_b)
  weights <- weights_from_pvals(pvals) * ifelse(coloc_results$h4 >= h4_threshold, 1, 0)
...
find_snp_and_connectedness_per_cluster <- function(
    coloc_groups,
    studies_to_colocalise,
    coloc_results,
    h4_threshold = posterior_prob_h4_threshold) {
  ...
  snp_and_connectedness_per_cluster <- lapply(components, function(component) {
    ...
    cumulative_lbf <- sapply(all_snps, function(snp) {
      return(sum(
        sapply(all_lbf_vectors, function(vec) {
          return(if (snp %in% names(vec)) vec[snp] else 0)
        }),
        na.rm = TRUE
      ))
    })
    ...
    total_pairs_with_h4 <- sum(coloc_results_per_cluster$PP.H4.abf >= h4_threshold, na.rm = TRUE)
    total_pairs_with_h3 <- sum(coloc_results_per_cluster$PP.H3.abf >= h4_threshold, na.rm = TRUE)
    ...
    return(data.frame(
      component = component,
      snp = snp,
      h4_connectedness = h4_connectedness,
      h3_connectedness = h3_connectedness
    ))
  }) |> dplyr::bind_rows()
