...
parser <- argparser::add_argument(
  ...
  help = "Coloc result file to save",
  type = "character"
)
parser <- argparser::add_argument(
  parser,
...
main <- function() {
  ...
  if ((!is.null(nrow(new_coloc_results)) && nrow(new_coloc_results) > 0) || args$force_clustering == TRUE) {
    ...
    if (can_cluster) {
      ...
      message(
        glue::glue(
          "{args$ld_block}: Marked {sum(coloc_results$false_positive, na.rm = TRUE)} false positives and ",
...
pairwise_coloc_analysis <- function(first_gwas, second_gwas) {
  ...
  if (length(first_lbf) < 50 || length(second_lbf) < 50) {
    return(NULL)
  }
...
cluster_coloc_results <- function(coloc_results, finemapped_studies, start_time) {
  ...
  message(
    glue::glue("{args$ld_block}: Clustering {nrow(coloc_results)} coloc results starting {diff_time_taken(start_time)}")
  )
