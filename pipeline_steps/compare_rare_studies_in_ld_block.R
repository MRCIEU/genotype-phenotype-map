...
compare_by_variant <- function(studies, variants, P_thresh) {
  ...
  for (var in variants_keep) {
    ...
    compare_wide <- rbind(compare_wide, data.frame(
      traits = paste(found_studies$unique_study_id, collapse = ", "), candidate_snp = var, min_ps = paste(found_studies$P, collapse = ", "), genes = paste(found_studies$GENE, collapse = ", ")))
  }
  ...
  colnames(compare_wide) <- c("traits","candidate_snp", "min_ps", "genes")
  return(compare_wide)
}
