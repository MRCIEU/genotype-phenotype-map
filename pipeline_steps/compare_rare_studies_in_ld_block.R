...
# In the absence of a complex LD structure surrounding rare variant associations,
# compare WES and WGS studies by variant identifer alone
...
main <- function() {
  ...
  standardised_studies$unique_study_id <- glue::glue(
    "{standardised_studies$study}_{file_prefix(standardised_studies$file)}"
  )
  ...
  compare_results <- compare_by_variant(
    studies = studies_to_compare,
    variants = variants,
    P_thresh = lowest_p_value_threshold
  )
...
compare_by_variant <- function(studies, variants, P_thresh) {
  variants_keep <- variants |>
    dplyr::filter(min_P <= P_thresh) |>
    dplyr::pull(SNP)
