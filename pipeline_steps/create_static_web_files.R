...
parser <- argparser::add_argument(
  parser,
  "--static_web_dir",
  help = "Directory for static web files (robots.txt, sitemap.xml)",
  type = "character"
)
parser <- argparser::add_argument(
  parser,
...
parser <- argparser::add_argument(
  ...
  help = "Static web files ready file",
  type = "character"
)
...
create_opengwas_map <- function(studies_db_file, static_web_dir) {
  ...
  jsonlite::write_json(
    updated_study_names,
    file.path(static_web_dir, "opengwas_ids.json"),
    auto_unbox = TRUE,
    pretty = TRUE
  )
  ...
  jsonlite::write_json(
    phenotype_id_map,
    file.path(static_web_dir, "phenotype_id_map.json"),
    auto_unbox = TRUE,
    pretty = TRUE
  )
