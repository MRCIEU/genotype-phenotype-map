...
parser <- argparser::add_argument(
  parser,
  "--bespoke_parsing",
  help = "Flag to signal bespoke parsing of besd format",
  type = "character",
  default = "none"
)
parser <- argparser::add_argument(
...
extract_cis_region <- function(study, p_value_threshold) {
  ...
  extract_cis_region <- glue::glue(
    "smr --beqtl-summary {study$study_location} ",
    "--query 1 ",
    "--probe {study$probe} ",
    "--cis-wind 2000 ",
    "--out {tmp_cis_region}"
  ...
  ld_block <- dplyr::filter(
    ld_blocks,
    chr == top_hit$CHR & start <= top_hit$BP & stop > top_hit$BP & ancestry == study$ancestry
  )
...
extract_trans_regions <- function(extracted_cis_snps, study, p_value_threshold) {
  ...
  if (!file.exists(glue::glue("{tmp_trans_snps}.txt"))) {
    return()
  }
  ...
  if (!file.exists(glue::glue("{tmp_trans_snps}.clumps"))) {
    return()
  }
  ...
  ld_block_strings <- apply(clumped_trans_snps, 1, function(clump) {
    ...
    ld_block <- dplyr::filter(
      ld_blocks,
      chr == as.numeric(clump["CHR"]) & start <= bp & stop > bp & ancestry == study$ancestry
    )
  ...
  if (is.data.frame(extracted_cis_snps) && nrow(extracted_cis_snps) == 1) {
    clumped_trans_snps <- dplyr::filter(
      clumped_trans_snps,
  ...
  extracted_trans_snps <- apply(clumped_trans_snps, 1, function(clumped_snp) {
    ...
    ld_block <- dplyr::filter(
      ld_blocks,
      chr == trans_chr & start <= trans_bp & stop > trans_bp & ancestry == study$ancestry
    )

    tmp_trans_region <- withr::local_tempfile()
    extract_region <- glue::glue(
      "smr --beqtl-summary {study$study_location} ",
      "--query 1 ",
      '--snp {clumped_snp["SNP"]} ',
    ...
    if (!file.exists(glue::glue("{tmp_trans_region}.txt"))) {
      return()
    }
    ...
    extracted_trans_hit <- data.frame(
      chr = as.character(trans_chr),
      bp = trans_bp,
      log_p = -log10(trans_p),
