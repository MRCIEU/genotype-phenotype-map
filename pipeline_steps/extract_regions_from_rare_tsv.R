...
parser <- argparser::add_argument(parser, '--extracted_study_location', help = 'Summary stats file', type = 'character')
parser <- argparser::add_argument(parser, '--bespoke_parsing', 'Flag to signal bespoke parsing of besd format', type = 'character', default = 'none')
parser <- argparser::add_argument(parser, '--extracted_output_file', help = 'Extracted output location', type = 'character')
...
main <- function() {
  ...
  if (nrow(gwas) == 0) {
    empty_extracted_snps <- data.frame(chr=character(),
                                      bp=numeric(),
                                      log_p=numeric(),
                                      ld_block=character(),
                                      file=character(),
...
split_into_regions <- function(gwas, study) {
  ...
  ld_block_strings <- apply(gwas, 1, function(extracted) {
    ...
    ld_block <- dplyr::filter(ld_blocks, chr == as.numeric(extracted['CHR']) & start <= bp & stop > bp & ancestry == study$ancestry)
