...
for (region in seq_len(nrow(ld_regions))) {
  ...
  ld_region_string <- paste0(ld_block_matrices_dir, ld_region$ancestry, "/", ld_region$chr, "/", ld_region$start, "_", ld_region$stop, ".ld")
  ...
  if (sum(!keep) > 0) {
    ld_data_string <- paste0(ld_block_matrices_dir, ld_region$ancestry, "/", ld_region$chr, "/", ld_region$start, "_", ld_region$stop, ".tsv")
