...
study_efos$num_clumped <- unlist(lapply(file_paths, function(x) {
  return(as.numeric(sub(" .*", "", system(paste("wc -l", x), intern = TRUE))))
}))
...
study_efos_unique <- study_efos[,
  {
    ...
    .SD[sample_size == max(sample_size), .(
      study_name,
      data_type,
      data_format,
      ancestry,
...
fwrite(
  as.data.frame(
    unlist(strsplit(
      paste(study_efos_unique[nchar(study_name_collapsed) > 0, study_name_collapsed], collapse = ","),
      ","
    ))
  ),
  "../pipeline_steps/data/more_ignore_studies.tsv",
  col.names = F
)
