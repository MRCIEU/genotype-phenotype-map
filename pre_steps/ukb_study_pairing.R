...
# 9056 studies (ukb and ebi - there are UKB derived sum stats in ebi- studies but there is no clear way to find which ones these are?)
# 6260 ukb only

# UKB studies to source .json files for (ukb-b and ukb-d)
ukb_studies <- common_studies |> filter(grepl("ukb-*", study_name)) |> pull(study_name)
...
#### ----------- Identify nonsense/non-heritable/'non-health' traits from common variant studies (not included in rare) ------------ #####
...
common_med <- common_studies |> filter(grepl("Treatment/medication code", trait)) |> 
    dplyr::select(data_type,study_name,trait) |>
    mutate(med = sub(".*: ","",trait)) #144

rare_med <- rare_studies |> filter(grepl("Treatment/medication code", trait)) |> 
    dplyr::select(data_type,study_name,trait, fieldID) |>
...
common_opp <- common_studies |> filter(grepl("Operative procedures", trait)) |> 
    dplyr::select(data_type,study_name,trait) |>
    mutate(opp = trait) #294

rare_opp <- rare_studies |> filter(grepl("Operative procedures", trait)) |> 
    dplyr::select(data_type,study_name,trait, fieldID) |>
...
common_opp_code <- common_studies |> filter(grepl("Operation code", trait)) |> 
    dplyr::select(data_type,study_name,trait) |>
    mutate(code = sub(".*: ","",trait)) #109

rare_opp_code <- rare_studies |> filter(grepl("Operation code", trait)) |> 
    dplyr::select(data_type,study_name,trait, fieldID) |>
...
common_studies_remaining <- common_studies |> 
  filter(!(study_name %in% c(
    ...
    common_rare_opp_code$study_name_common))) #1485 remaining
...
rare_studies_remaining <- rare_studies |> 
  filter(!(study_name %in% c(
    ...
    rare_med$study_name,
    rare_opp$study_name,
    rare_opp_code$study_name))) #7536 remaining

# ----------- Amalgamated diseases ---------- #
rare_union <- rare_studies |> filter(grepl("union", trait)) |> 
    dplyr::select(data_type,study_name,trait, fieldID) |>
...
common_rare_match <- inner_join(common_studies_remaining[,c("data_type","study_name","trait")], rare_studies[,c("data_type","study_name","trait","fieldID")],
...
rare_studies_remaining <- rare_studies_remaining |> 
  filter(!(study_name %in% c(
    common_rare_fields$study_name_rare, common_rare_modmatch2$study_name_rare))) #7101

# ----------- Manual EFO matching for remainder --------- #
efos <- readr::read_csv(url("https://raw.githubusercontent.com/MRCIEU/finetune-efo/refs/heads/main/manual-mapping/trait-efo.csv"))
