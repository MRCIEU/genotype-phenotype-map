...
## Rare variants
Rare variant annotation was performed for 1,679,940 WES variants with MAF < 1% and MAC > 5 in [Genebass](https://app.genebass.org/), extracted using the `pull_rarevarids_fromgenebass.py`

`format_rare_varids.sh` was used to reformat variant information to VEP ensembl input:\
`CHR POS_START POS_END REF/ALT STRAND NAME`\
All imputed variants are reported on the top strand. Variant name is in the format `chr_bp_A1_A2` with alleles ordered alphabetically. `update_indelpositions.R` was used to update the chromosomal positions and ref/alt coding for short insertions and deletions as previously.

VEP was run with the parameters:

```
vep -i genebass_rarevariant_info_hg38_forVEP_posupdate.txt \
--cache \
--assembly GRCh38 \
--af_1kg \
--sift b \
...
--biotype \
--check_ref \
--skipped_variants_file vep_rare_skipped.txt \
--pick \
--no_stats \
--force_overwrite \
-v \
-o vep_rare_variantannotations_hg38.txt
```
