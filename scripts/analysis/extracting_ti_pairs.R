...
#data.table::fwrite(term_tab, file = file.path(data_dir,"target-indication_data/ericminike_traittomesh.csv"), quote = TRUE)
...
# The assumption here is that the phenotypic associations sourced in Minikel et al. are the more exhaustive set, and should
...
forAI <- cbind(gpmap_trait = gpmap_traits$trait_name_gpmap[1:nrow(term_tab)], original_trait = term_tab$original_trait)
#data.table::fwrite(forAI, file = file.path(data_dir,"target-indication_data/ericminike_gpmap_traitstomatch.csv"), quote = TRUE)
...
### ----- STEP 4: -----
# Retain only the GPMAP traits which can be matched to a drug-target indication in ti_pairs (based on MeSH term similarity of >=0.8)
...
mesh_terms <- data.frame(mesh_id = c(ti_pairs$indication_mesh_id, term_tab$mesh_id), mesh_term = c(ti_pairs$indication_mesh_term, term_tab$mesh_term)) |> unique()
mesh_match_wterms <- mesh_match |> dplyr::left_join(mesh_terms, by = c("meshcode_a" = "mesh_id")) |>
   dplyr::left_join(mesh_terms, by = c("meshcode_b" = "mesh_id"), suffix = c("_a","_b")) |> na.exclude()
...
mesh_match_highconf <- mesh_match |> 
  dplyr::filter(meshcode_a %in% i_mesh | meshcode_b %in% i_mesh,
                comb_norm >= 0.8)

# Which MeSH code is the indicator code, and which is the matched code
i_idx <- apply(mesh_match_highconf, 1, function(x){which(x %in% i_mesh)})
i_idx_both <- which(lapply(i_idx, length) == 2) # Both codes are in the indicator set
i_idx <- sapply(i_idx, function(x){x[1]})

mesh_match_highconf <- mesh_match_highconf |> 
  dplyr::mutate(indication_mesh_id = ifelse(i_idx == 1, meshcode_a, meshcode_b),
  ...
  rbind(data.frame(indication_mesh_id = mesh_match_highconf$meshcode_b[i_idx_both],
                   matched_mesh_id = mesh_match_highconf$meshcode_a[i_idx_both],
                   comb_norm = mesh_match_highconf$comb_norm[i_idx_both])) |> # Add inverse for rows with two indicator Mesh IDs 
...
# Manually check the pairing of GPMAP traits to original traits conducted above (based on AI reported best_match and confidence)
...
gpmap_matched_filtered <- data.table::fread(file.path(data_dir,"target-indication_data/gpmap_matched_traits_filtered_corrected.csv")) 
...
gpmap_matched_filtered <- gpmap_matched_filtered |> dplyr::left_join(mesh_match_highconf, by = c("mesh_id" = "matched_mesh_id"), relationship = "many-to-many")
...
# Bind GPMAP association studies to ti_pairs (keeping pairs for which both the indication and the target are included in GPMAP studies)
ti_pairs_gpmap <- dplyr::left_join(ti_pairs[,c("ti_uid", "target",
                                   ...
                                   gpmap_matched_filtered, by = c("indication_mesh_id","indication_mesh_term"), relationship = "many-to-many") |>
...
length(unique(ti_pairs_gpmap$ti_uid)) #18932/25713 (74%) unique T-I pairs tested in Minikel et al. have genetic evidence in GPMAP 
...
# data.table::fwrite(ti_pairs_gpmap, file.path(data_dir, "target-indication_data/target-indicationpairs_gpmapevidence.tsv"), sep = "\t")
