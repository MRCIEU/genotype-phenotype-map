...
meta$bm$fieldID <- ifelse(
  grepl("\\([0-9]+\\)$", meta$bm$trait_original),
  sub(".*\\(([0-9]+)\\)$", "\\1", meta$bm$trait_original),
  NA
)
...
bm_selfreported <- grep(
  "^Non cancer illness code self reported|Self Report",
  meta$bm$trait_original,
  value = T
) # 365 OUT
bm_continuous <- meta$bm |>
  dplyr::filter(category == "continuous") |>
  pull(trait_original) # 289 KEEP
bm_medication <- grep(
  "Treatment or Medication|medication for",
  meta$bm$trait_original,
  ignore.case = T,
  value = T
) # 257 OUT

bm_out_studies <- meta$bm |>
  dplyr::filter(trait_original %in% c(bm_selfreported, bm_medication)) |>
  dplyr::pull(study_name) # 622 to remove
...
gb_binary <- meta$gb |>
  dplyr::filter(category == "binary") |>
  pull(trait_original) # 725 OUT
gb_opps <- grep("operat|^Treatment|^Medication", meta$gb$trait_original, value = T, ignore.case = T) # 2139 KEEP
gb_categorical <- meta$gb |>
  dplyr::filter(category == "categorical" & !(trait_original %in% gb_opps)) |>
  pull(trait_original) # 432 OUT

gb_continuous <- meta$gb |>
  dplyr::filter(category == "continuous") |>
  pull(trait_original) # 1233

# Remove continuous variables alread in bm (based on field ID)
bm_continuous_match <- meta$bm |>
  dplyr::filter(trait_original %in% c(bm_continuous)) |>
  dplyr::pull(fieldID) |>
  na.exclude()
gb_continuous_match <- meta$gb |>
  dplyr::filter(category == "continuous" & fieldID %in% bm_continuous_match) |>
  pull(trait_original) # 134 OUT

gb_out_studies <- meta$gb |>
  dplyr::filter(trait_original %in% c(gb_binary, gb_categorical, gb_continuous_match)) |>
  dplyr::pull(study_name) # 1300 to remove

# For azexwas studies:
# Remove binary disease traits (keeping only bm ICD10 codes as master studies, there is too much duplication here otherwise) # nolint: line_length_linter.
...
az_proteins <- meta$az |>
  ...
  pull(trait_original) # 2941 KEEP
az_nmr <- meta$az |>
  filter(grepl("^NMR ", trait)) |>
  pull(trait_original) # 325 KEEP

# Remove remaining continuous traits (duplicated with gb)
az_continuous <- meta$az |>
  dplyr::filter(category == "continuous", !(trait_original %in% c(az_proteins, az_nmr))) |>
  pull(trait_original) # 1602 OUT

az_out_studies <- meta$az |>
  dplyr::filter(!(trait_original %in% c(az_union, az_block, az_proteins, az_nmr))) |>
  dplyr::pull(study_name) # 7031 to remove
...
trait_mapping <- read.csv(
  "https://raw.githubusercontent.com/MRCIEU/finetune-efo/refs/heads/main/manual-mapping/trait-efo.csv",
  header = T,
  row.names = 1
)
...
data.table::fwrite(
  mapping_rare,
  "/local-scratch/projects/genotype-phenotype-map/data/trait_cleaning/deduplicated_rarestudies.tsv",
  sep = "\t"
)
...
# meta_filt_r4 <- meta_filt_r3 |> dplyr::filter(study_name %in% c(mapping_rare$study_name,rare_proteins,rare_nmr,rare_opp)) # 14173 rare studies remaining # nolint: line_length_linter.
...
# data.table::fwrite(out, "/local-scratch/projects/genotype-phenotype-map/data/trait_formatting/deduplicated_rarestudies.tsv", sep = "\t") # nolint: line_length_linter.
