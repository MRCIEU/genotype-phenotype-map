...
additional_rare_variants_columns <- function(studies, rare_variants) {
  rare_variants <- rare_variants |>
    tidyr::separate(unique_study_a, '_', into=c('study_a'), remove=F) |>
    tidyr::separate(unique_study_b, '_', into=c('study_b'), remove=F)

  study_a_match <- match(rare_variants$study_a, studies$unique_study_id)
  study_b_match <- match(rare_variants$study_b, studies$unique_study_id)

  rare_variants$trait_a <- studies$trait[study_a_match]
  rare_variants$trait_b <- studies$trait[study_b_match]
  rare_variants$data_type_a <- studies$data_type[study_a_match]
  rare_variants$data_type_b <- studies$data_type[study_b_match]
  rare_variants$gene_a <- studies$known_gene[match(rare_variants$study_a, studies$unique_study_id)]
  rare_variants$gene_b <- studies$known_gene[match(rare_variants$study_b, studies$unique_study_id)]
  rare_variants$tissue_a <- studies$tissue[match(rare_variants$study_a, studies$unique_study_id)]
  rare_variants$tissue_b <- studies$tissue[match(rare_variants$study_b, studies$unique_study_id)]
...
rare_results <- vroom::vroom(glue::glue('{last_result_dir}/rare_results.tsv'), show_col_types=F)
...
phenotype_data <- function() {
  study_of_interest <- 'ebi-a-GCST90029025'
  rare_study_of_interest <- 'ukb-wes-az-00010232'
  ...
  rare_results <- additional_rare_variants_columns(studies_processed, rare_results)
  rare_results <- rare_results |>
    dplyr::filter(study_a == rare_study_of_interest | study_b == rare_study_of_interest)

  combined_data <- list(
    studies = only_interesting_blocks_by_snp,
    colocs = coloc_results_for_study,
    rare_variants = rare_results
  )
...
snp_data <- function() {
  specific_snp <- "11:201584_A_T" #this one has 168 entries
