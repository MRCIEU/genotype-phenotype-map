...
metadata_file <- file.path(rawdata_dir,"downloads/genebass/rare/ukb-wes-gb-studies.tsv")
...
#ld_split <- lapply(ld_split, function(x){
	...
	#return(chr_join)
#})
# Updated ld blocks now have these ^
...
# MAF filtering already perfomed in 02_pullfromgenebass.py
study_ancestry <- "EUR"

input_columns <- c("Variant","Chromosome","Base pair location","Effect allele","Other allele","AF","BETA","SE","Pvalue")
...
check_gwas <- function(study){
    ...
    study <- study[grepl(paste0("chr",1:22, collapse = "|"), study$locus),]

	# Add missing columns
	study$Chromosome <- as.numeric(sub("chr(.*):.*","\\1",study$locus))
	study$`Base pair location` <- as.numeric(sub("chr.*:","",study$locus))
	study$`Effect allele` <- sub(".*_.*\\/(.*$)","\\1",study$markerID)
	study$`Other allele` <- sub(".*_(.*)\\/.*$","\\1",study$markerID)

	# Update variant ID
	study$Variant <- gsub("chr","",gsub("/",":",gsub("_",":",study$markerID)))
