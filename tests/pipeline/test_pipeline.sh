...
if [[ $EXTRA_ARG =~ "delete" ]]; then
  ...
  rm -r $RESULTS_DIR/latest/studies_processed.tsv.gz
...
export APPTAINER_VARS="-B /local-scratch -B /projects -B /home/$(whoami) -B /local-scratch/tmp:/tmp -B $(pwd):/home/pipeline -B /usr/bin --env TIMESTAMP=$TIMESTAMP --pwd /home/pipeline"
