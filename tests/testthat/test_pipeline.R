...
test_that("Pipeline execution and file validation", {
  ...
  test_that("traits_processed and studies_processed are valid", {
    expect_true(
      file.exists(expected_tsv_files$traits_file),
      info = glue::glue("File should exist: {expected_tsv_files$traits_file}")
    )
    expect_true(
      file.size(expected_tsv_files$traits_file) > 0,
      info = glue::glue("File should not be empty: {expected_tsv_files$traits_file}")
    )

    expect_true(
      file.exists(expected_tsv_files$studies_file),
      info = glue::glue("File should exist: {expected_tsv_files$studies_file}")
    )
    expect_true(
      file.size(expected_tsv_files$studies_file) > 0,
      info = glue::glue("File should not be empty: {expected_tsv_files$studies_file}")
    )
    ...
    expect_true(
      nrow(joined_studies_traits) == nrow(studies) && nrow(joined_studies_traits) > 0,
      info = "All studies should have an associated trait"
    )

    sapply(joined_studies_traits$extracted_location, function(extracted_location) {
      expect_true(
        dir.exists(extracted_location),
        info = glue::glue("Extracted location should exist: {extracted_location}")
      )
  ...
  test_that("study_extractions are valid", {
    expect_true(
      file.exists(expected_tsv_files$study_extractions_file),
      info = glue::glue("File should exist: {expected_tsv_files$study_extractions_file}")
    )
    expect_true(
      file.size(expected_tsv_files$study_extractions_file) > 0,
      info = glue::glue("File should not be empty: {expected_tsv_files$study_extractions_file}")
    )
    ...
    lapply(seq_len(nrow(common_extractions)), function(i) {
      ...
      expect_true(
        file.size(extraction$svg_file) > 0,
        info = glue::glue("Extracted file should not be empty: {extraction$svg_file}")
      )
      expect_true(
        file.exists(extraction$file_with_lbfs),
        info = glue::glue("Extracted file should exist: {extraction$file_with_lbfs}")
      )
      expect_true(
  ...
  test_that("LD Block directories contain expected files", {
    ...
    expected_rare_files <- c(
      "extracted_studies.tsv",
      "standardised_studies.tsv",
      "compare_rare_results.tsv",
      "compare_rare_cached_studies.tsv"
    )
  ...
  test_that("Study directory data is properly structured", {
    ...
    for (i in seq_len(nrow(studies))) {
      ...
      expect_true(
        nrow(extracted_snps) == length(files_in_extracted_dir),
        info = glue::glue("Extracted SNPs file should not be empty")
      )
    }
  })
  ...
  test_that("Rare results are valid", {
    expect_true(
      file.exists(expected_tsv_files$rare_results_file),
      info = glue::glue("File should exist: {expected_tsv_files$rare_results_file}")
    )
  ...
  test_that("Static web files are ready", {
    expect_true(
      file.exists(expected_static_web_files$pipeline_summary_file),
      info = glue::glue("File should exist: {expected_static_web_files$pipeline_summary_file}")
    )
    expect_true(
      file.size(expected_static_web_files$pipeline_summary_file) > 0,
      info = glue::glue("File should not be empty: {expected_static_web_files$pipeline_summary_file}")
    )
    expect_true(
      file.exists(expected_static_web_files$opengwas_ids_file),
      info = glue::glue("File should exist: {expected_static_web_files$opengwas_ids_file}")
    )
    expect_true(
      file.size(expected_static_web_files$opengwas_ids_file) > 0,
      info = glue::glue("File should not be empty: {expected_static_web_files$opengwas_ids_file}")
    )
  })
  ...
  test_that("DB files are valid", {
    ...
    expect_true(
      file.exists(expected_db_files$studies_db_file),
      info = glue::glue("File should exist: {expected_db_files$studies_db_file}")
    )
    expect_true(
    ...
    coloc_pairs_full_conn <- duckdb::dbConnect(
      duckdb::duckdb(shared_home = TRUE), expected_db_files$coloc_pairs_full_db_file
    )
    ...
    table_data <- DBI::dbGetQuery(
      coloc_pairs_significant_conn,
      glue::glue("SELECT count(*) as count FROM {coloc_pairs_significant_table$name}")
    )
    expect_true(
      table_data$count > 0,
