...
test_that("Pipeline worker runs for TSV file", {
  ...
  compiled_coloc_pairwise_results <- vroom::vroom(glue::glue("{extracted_study_dir}/compiled_coloc_pairwise_results.tsv"), show_col_types = FALSE)
  expect_true(nrow(compiled_coloc_pairwise_results) > 0, info = "Compiled coloc pairwise results file should have rows")

  compiled_extracted_studies <- vroom::vroom(glue::glue("{extracted_study_dir}/compiled_extracted_studies.tsv"), show_col_types = FALSE)
  expect_true(nrow(compiled_extracted_studies) > 0, info = "Compiled extracted studies file should have rows")

  compiled_coloc_clustered_results <- vroom::vroom(glue::glue("{extracted_study_dir}/compiled_coloc_clustered_results.tsv"), show_col_types = FALSE)
  expect_true(nrow(compiled_coloc_clustered_results) > 0, info = "Compiled coloc clustered results file should have rows")

  compiled_associations <- vroom::vroom(glue::glue("{extracted_study_dir}/compiled_associations.tsv"), show_col_types = FALSE)
  expect_true(nrow(compiled_associations) > 0, info = "Compiled associations file should have rows")
  expect_true(nrow(compiled_associations) == nrow(compiled_coloc_clustered_results), info = "Compiled associations file should have the same number of rows as compiled coloc clustered results file")
})
